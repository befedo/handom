...
isaac seed num
  | num < 1   = []
  | otherwise = take num stream
  where
  stream      = genStream $ genkey (listToArray 0 seed emptyarr)
  ...
  listToArray n    (x:xs) arr = listToArray (n+1) xs newarr
    where
    newarr  = arr // [(arrelem, (arr!arrelem) .|. val)]
...
genStream arr = aux arr 0 1 1 0
  where
  aux :: IsaacArray -> Word32 -> Word32 -> Word32 -> Word32 -> [Word32]
  aux arr aa bb cc 256  = aux arr aa (bb+cc+1) (cc+1) 0
  aux arr aa bb cc iter = res : aux newarr newaa res cc (iter+1)
    where
    newaa  = (aa `xor` (aa `shift` qq)) + (arr ! ((iter + 128) `mod` 256))
               where
               qq = case iter `mod` 4 of 0 -> 13;   1 -> -6;   2 -> 2;   3 -> -16
    x      = arr ! iter
...
mix a = aux a 0
  ...
  aux [a, b, c, d, e, f, g, h] x = aux [b+c,c,newa+d,e,f,g,h,newa] (x+1)
    where
    newa = a `xor` (b `shift` (-currx))
    currx = case x of 0 -> -11;  1 -> 2;  2 -> -8;  3 -> 16; 4 -> -10;  5 -> 4;  6 -> -8;  7 -> 9
...
pass tmix arr i   = pass newtm (arr // [(intToWord32 (off+i), newtm!!off) | off <- [0..7]]) (i+8)
                      where
                      newtm = mix $ zipWith (+) tmix [arr!intToWord32 (i+off) | off <- [0..7]]
...
mixArray tmix arr k i   = mixArray newtm (arr // [(intToWord32 (i+off), newtm!!off) | off <- [0..7]]) k (i+8)
                            where
                            newtm = mix $ zipWith (+) tmix [k!intToWord32 (i+off) | off <- [0..7]]
